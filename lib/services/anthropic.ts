...
export class AnthropicService {
  ...
  private getTextFromResponse(response: Anthropic.Message): string {
  ...
  private zodSchemaToJsonDescription(schema: z.ZodObject<z.ZodRawShape>): string {
    // Generate a human-readable description of the expected JSON schema
    const shape = schema.shape;
    const fields: string[] = [];
    for (const [key, value] of Object.entries(shape)) {
      fields.push(`  "${key}": ${this.describeZodType(value as z.ZodTypeAny)}`);
    }
    return `{\n${fields.join(',\n')}\n}`;
  }

  private describeZodType(schema: z.ZodTypeAny): string {
    if (schema instanceof z.ZodString) return 'string';
    if (schema instanceof z.ZodNumber) return 'number';
    if (schema instanceof z.ZodBoolean) return 'boolean';
    if (schema instanceof z.ZodArray) return 'array';
    if (schema instanceof z.ZodNullable) return `${this.describeZodType((schema as z.ZodNullable<z.ZodTypeAny>)._def.innerType)} | null`;
  ...
  async extractStructuredDataOriginal(
    ...
    try {
      ...
      const schemaDescription = this.zodSchemaToJsonDescription(schema);

      const response = await this.client.messages.create({
        model: 'claude-sonnet-4-20250514',
        max_tokens: 8192,
        system: `You are an expert data extractor. Extract the requested information from the provided content with high accuracy.
...
${schemaDescription}`,
      ...
      const messageContent = this.getTextFromResponse(response);
      const jsonStr = this.extractJsonFromText(messageContent);
  ...
  async extractStructuredDataWithCorroboration(
    ...
    try {
      ...
      const response = await this.client.messages.create({
        model: 'claude-sonnet-4-20250514',
        max_tokens: 8192,
        system: `You are an expert data extractor. Extract information with evidence from each source.
...
${schemaDescription}`,
      ...
      console.log('[ANTHROPIC] Raw corroboration response sample:', {
      ...
      console.error('Anthropic corroborated extraction error:', error);
  ...
  async extractStructuredData(
    ...
    try {
      ...
      const response = await this.client.messages.create({
        model: 'claude-sonnet-4-20250514',
        max_tokens: 2000,
        system: `Extract structured data based on the prompt. Return valid JSON matching this schema:
${schemaDescription}
  ...
  async generateSearchQueries(
    ...
    try {
      const response = await this.client.messages.create({
        model: 'claude-sonnet-4-20250514',
        max_tokens: 1000,
        system: `Generate effective search queries to find information about "${targetField}" for the given entity.
...
You MUST respond with ONLY valid JSON like: {"queries": ["query1", "query2"]}`,
        messages: [
          {
            role: 'user',
...
Generate new search queries to find: ${targetField}`,
  ...
  async generateSearchQuery(question: string, context?: Record<string, unknown>): Promise<string> {
    try {
      ...
      const systemPrompt = `You are a search query optimizer. Convert natural language questions into effective search queries.
...
Return ONLY the search query, nothing else.`;
      ...
      if (conversationHistory.length > 0) {
        ...
        messages.push({
          role: 'assistant',
          content: 'I understand the conversation context. Please provide the question you want me to convert into a search query.'
        });
      }
      ...
      return this.getTextFromResponse(response).trim() || question;
    } catch (error) {
  ...
  async answerFromTableData(
    ...
    try {
      const systemPrompt = `You are a data analyst examining enriched table data. Your job is to answer questions ONLY if the specific information is present in the data.
...
- Be specific with your answer, citing actual data values`
      ...
      const answer = this.getTextFromResponse(response).trim();
      ...
      if (isGenericAnswer) {
        console.log('[ANTHROPIC] Table data check: NOT_FOUND (generic or not found)');
        return { found: false };
      }

      console.log('[ANTHROPIC] Table data check: FOUND -', answer.substring(0, 100));
  ...
  async selectBestSource(
    ...
    try {
      const response = await this.client.messages.create({
        model: 'claude-sonnet-4-20250514',
        max_tokens: 50,
        system: 'You are a source evaluator. Select the best source to answer the question. Return JSON with {index: number}. Return ONLY the JSON, nothing else.',
        messages: [
          {
            role: 'user',
  ...
  async generateConversationalResponse(
    ...
    try {
      ...
      const systemPrompt = `You are a helpful research assistant. Answer questions conversationally based on the provided content and previous conversation context.
...
Be concise but informative. If the content doesn't contain the answer, say so. When citing dates or time-sensitive information, consider the current date for context.`;
      ...
      return this.getTextFromResponse(response).trim() || "I couldn't find a clear answer to your question.";
    } catch (error) {
